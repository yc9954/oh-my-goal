...
When independent worker lanes are useful, use `scripts/team-runtime.mjs`. `scripts/omx-team-core.mjs` owns the OMX-derived Team contracts: task decomposition, worker identity, `schema_version: 2` manifest, task files, worker packets, `.omg/runtime/team/<team>/` state, status, collection, and shutdown. `team-runtime.mjs` adds only launch/status transport through cmux or tmux. Workers still must not own the Codex goal.
