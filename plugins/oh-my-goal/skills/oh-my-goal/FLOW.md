...
The first `$oh-my-goal <objective>` response is intake only. Run focused preflight, start the sequential OMX-style intake question runtime, ask exactly one question with ambiguity score, then stop. Harness generation and implementation happen only after all intake questions are answered or the user explicitly approves defaults.

## Phase Router

| Phase | Read | Do | Do not |
| --- | --- | --- | --- |
| `INTAKE_PENDING` | `flows/00-entrypoint.md`, `flows/01-intake-gate.md`, `templates/first-turn-response.md`, `templates/intake-fallback.md` | inspect focused context, start or continue sequential OMX question runtime, ask one question at a time | create files, run harness generator, code, create goal |
