# Oh My Goal-Derived Patterns

Oh My Goal preserves selected Oh My Goal behaviors without requiring the Oh My Goal runtime.
...
- Use the canonical Oh My Goal question schema from `src/question/types.ts`: `questions[]`, `single-answerable`, `multi-answerable`, `answers[]`, and `selected_values`.
...
- Use `scripts/team-core.mjs` for the plugin-local Oh My Goal Team state contract and `scripts/team-runtime.mjs` for cmux/tmux worker panes, status, result collection, and shutdown.
